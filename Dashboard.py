...
def load_data():
    ...
    if 'gsheets_conn' not in st.session_state:
        try:
            ...
            st.session_state.gc = gspread.service_account_from_dict(service_account_info)
            st.session_state.doc = st.session_state.gc.open_by_url(spreadsheet_url)
        except Exception as e:
            st.error(f"Google Sheets 연결 실패: {str(e)}")
            st.stop()

    # 2. 데이터 로드 부분만 캐싱 (데이터 자체는 캐시 유지)
    @st.cache_data(ttl=3600)  # 1시간 캐시
    def _load_sheets_data(_doc):
        worksheets = _doc.worksheets()
        sheets = {}
        for ws in worksheets:
            name = ws.title
            if name != "시트1" and not name.startswith("Cache_"):
                data = ws.get_all_values()
                if data:
                    sheets[name] = pd.DataFrame(data[1:], columns=data[0])
        return sheets

    # 3. 캐시 시트 처리 (캐시에서 제외)
    try:
        cache_ws = st.session_state.doc.worksheet("Cache_Results")
    except gspread.WorksheetNotFound:
        cache_ws = st.session_state.doc.add_worksheet("Cache_Results", 1000, 20)
        cache_ws.update('A1:E1', [['sheet_name', 'data_hash', 'additional_question', 'chart_title', 'analysis_result']])

    # 4. 캐시 데이터 로드
    cache_data = cache_ws.get_all_values()
    ...
    return _load_sheets_data(st.session_state.doc), model, st.session_state.doc, cache_ws, cache_df

# 데이터 로드
sheets, model, doc, cache_worksheet, cache_df = load_data()
# 시트가 없는 경우 오류 메시지 표시
if not sheets:
    st.error("로드할 데이터가 없습니다. Google Sheets 연결 상태를 확인하세요.")
    st.stop()
...
def find_cached_result(cache_df, target_sheet, target_hash, target_question):
    """
    시트 이름과 데이터 해시로 캐시 검색
    Returns:
        tuple: (analysis_result, chart_title) or (None, "") if not found
    """
    try:
        # 필수 컬럼 확인
        if not {'sheet_name', 'data_hash', 'additional_question'}.issubset(cache_df.columns):
#            st.success(cache_df.columns)
            return None

        # 정확한 비교를 위한 타입 정리 (모두 문자열로 비교)
        target_hash = str(target_hash).strip()
        target_sheet = str(target_sheet).strip()
        target_question = str(target_question).strip()

        # 캐시 데이터 처리
        cache_df = cache_df.copy()
        cache_df['data_hash'] = cache_df['data_hash'].astype(str).str.strip()
        cache_df['sheet_name'] = cache_df['sheet_name'].astype(str).str.strip()
        cache_df['additional_question'] = cache_df['additional_question'].astype(str).str.strip()

        # 검색 수행
        mask = (
            (cache_df['sheet_name'] == target_sheet) &
            (cache_df['data_hash'] == target_hash) &
            (cache_df['additional_question'] == target_question)
        )
        matching_rows = cache_df[mask]

        if not matching_rows.empty:
            return matching_rows.iloc[0].get('analysis_result')

    except Exception as e:
        st.error(f"캐시 검색 오류: {str(e)}")

    return None

if cache_df.empty:
    cache_df = pd.DataFrame(columns=['sheet_name', 'data_hash', 'additional_question', 'chart_title', 'analysis_result'])
...
if st.button("확인"):
    ...
    cached_result = find_cached_result(
        cache_df=cache_df,
        target_sheet=selected_sheet,
        target_hash=data_hash,
        target_question = additional_question
    )
                ...
                try:
                    ...
                    try:
                        ...
                        new_df = pd.DataFrame(new_row,
                                              columns=['sheet_name', 'data_hash', 'additional_question', 'chart_title',
                                                       'analysis_result'])
