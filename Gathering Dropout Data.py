    ...
    for stat_table in soup.find_all('통계표'):
        ...
        if table_name != '학교생활 만족도':
            ...
            if df_filtered.empty:
                # 검색할 단어 리스트
                keywords = ['GNI_증가율', '고졸', '_실질', '백분율', '요일평균', '_조이혼율', '합계', '고등학교_소계', '고등학교_%', '소비자물가상승률(%)']
                # 정규식 패턴 생성 (단어들을 |로 연결)
...
# 기존 컬럼명
old_columns = [
"연도", "1인당 GNI_증가율(달러화) 2)_만 원, 달러, %", "명목 및 실질 경제성장률 1)2)_실질 경제성장률_%", "학교급별 교원 1인당 학생 수 1)2)3)_고등학교_소계_명", "교육 수준별 임금수준 1)2)3)4)_고졸_%", "가구주연령 및 소득수준별 교육비 부담도 1)_전체_%", "성·연령·소득수준별 긍정적 정서경험 1)2)_전체_%", "기초학력 미달률 1)", "학교급별 다문화학생 1)2) 비율 3)_전체_%", "부패인식 지수와 부패인식 지수 국제순위_한국순위 백분율 4)_점, 위, 개국, %", "학교급별 사교육 참여율_고등학교_전체_%", "학교급별 학생 1인당 월평균 사교육비 1)_고등학교_전체_만 원", "성·연령·소득수준별 삶의 만족도 1)2)_전체_%", "범죄유형별 소년 범죄자율 1)2)_전체_명/10만 명", "연령 및 소득수준별 소득 만족도_전체_%", "실업률 1)_전체_%", "성 및 연령별 우울감 경험률 1)_전체_%", "성·연령·소득수준별 일의 가치 인식 1)2)_전체_%", "성·교육수준·직업별 전공직업일치도_전체_%", "이혼건수와 조이혼율_조이혼율 1)_건, 건/1,000명", "교육단계별 진학률_전체_%", "학교생활 만족도_교육내용_%", "학교생활 만족도_교육방법_%", "학교생활 만족도_교우관계_%", "학교생활 만족도_교사와의 관계_%", "학교생활 만족도_학교시설_%", "학교생활 만족도_학교주변 환경_%", "학교생활 만족도_전반적 학교생활_%", "학교폭력 피해율_전체_%", "학교급별 학급당 학생 수_고등학교_소계_명", "학교급별 학업중단율_고등학교_%", "여가시간 충분도_요일평균_%", "소비자물가총지수와 주요 품목별 소비자물가상승률"
]

# 수정 컬럼명
new_columns = [
"년도", "1인당 GNI_증가율", "실질 경제성장률", "교원 1인당 학생 수", "교육 수준별 임금수준", "교육비 부담도", "긍정적 정서경험", "기초학력 미달률", "다문화학생 비율", "부패인식", "사교육 참여율", "1인당 월평균 사교육비", "삶의 만족도", "소년 범죄자율", "소득 만족도", "실업률", "우울감 경험률", "일의 가치 인식", "전공직업일치도", "조이혼율", "진학률", "교육내용 만족도", "교육방법 만족도", "교우관계 만족도", "교사와의 관계 만족도", "학교시설 만족도", "학교주변 환경 만족도", "학교생활 만족도", "학교폭력 피해율", "고등학교 학생 수", "학업중단율", "여가시간 충분도", "소비자물가상승률"
]
...
merged_df['소년 범죄자율'] = merged_df['소년 범죄자율'] / 10000
